class Utils {
	...
	is_same_date(dateString) {
		let event_date = new Date(dateString),
			now = new Date();
		return event_date.getUTCFullYear() == now.getUTCFullYear()
			&& event_date.getUTCMonth() == now.getUTCMonth()
			&& (event_date.getUTCDate() == now.getUTCDate()
			|| event_date.getUTCDate() == now.getUTCDate()-1
			|| event_date.getUTCDate() == now.getUTCDate()+1);
	}
}
